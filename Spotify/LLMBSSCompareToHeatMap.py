...
def NarrowDownDFLLM(df,contextFilePath, featuresToGet):
    ...
    for feature in valid_cols:
        featureDicLLM[feature] += pointCounter
        pointCounter -= 1
...
def save_results(results,ModelName,p,k,trials):
    ...
    if "corr matched features" in results[ModelName]:
        output["features matched to correlation matrix"] = results[ModelName]["corr matched features"]
...
def run_trial(model,df,y,seed,DfFeatureAmount,results,SvmFeatureAmount,contextFile=None,otherFeatureNames=None):
    ...
    match model:
        ...
        case "Rand":
            ...
            for feature in currdf.columns:
                featureDicRand[feature] += 1
...
#     heatmapFeatures = list(["Spotify Playlist Count",
...
#     if "corr matched features" not in results[f"{model}train"]:
#         results[f"{model}train"]["corr matched features"] = list()
#     results[f"{model}train"]["corr matched features"].append(match_features(ChosenFeatureNames,heatmapFeatures))
...
featureDicLLM = {key: 0 for key in df.columns}
featureDicRand = {key: 0 for key in df.columns}
...
#heatmap
print(featureDicLLM)
print(featureDicRand)

# Convert to DataFrame
importance_df = pd.DataFrame.from_dict(featureDicLLM, orient='index', columns=['Importance'])
...
plt.title("LLM Selected Features")
plt.ylabel("Features")
plt.tight_layout()
...
#Rand
importance_df = pd.DataFrame.from_dict(featureDicRand, orient='index', columns=['Importance'])
...
plt.title("Rand Selected Features")
plt.ylabel("Features")
plt.tight_layout()
plt.xticks([0.5], ['Importance'], rotation=0)
plt.show()